...
public class GetListOfVoucherProducts implements Processor {
	static Logger log = Logger.getLogger(GetListOfVoucherProducts.class.getName());
		...
		if (dataListObject.has("apply") && dataListObject.getString("apply").equalsIgnoreCase("SPECIFIC_PRODUCTS")) {
			while (hasMoreProduct) {
				if (dataListObject.has("id")) {
					JSONArray productList = getVoucherProducts(exchange, dataListObject.getString("id"), failedCount);
					mergeSKUIDList(productsList, productList);
				}
			}
			dataListObject.put("products", productsList);
		}
		exchange.setProperty("voucherItemWithProduct", dataListObject);
	}
	...
	public JSONArray getVoucherProducts(Exchange exchange, String promotionID, int failedCount) {
		...
		String response = "";
		try {
			response = NewLazadaConnectionUtil.callAPI(hostURL, "/promotion/voucher/products/get", accessToken, map, "",
					queryParams, "GET", clientID, clientSecret);
			if (response.isEmpty() || !response.startsWith("{")) {
				hasMoreProduct = false;
				log.error("Failed to get list of voucher products for accountNumber :"
			...
			if (serviceResponse.has("code") && serviceResponse.getString("code").equals("0")) {
				if (!serviceResponse.has("data") || serviceResponse.getJSONObject("data").length() == 0) {
					...
					if (failedCount < MAX_RETRY_COUNT) {
						log.info("Retrying to get list of voucher products, data is empty " + (failedCount + 1));
						failedCount = failedCount + 1;
						return getVoucherProducts(exchange, promotionID, failedCount);
					} else {
						hasMoreProduct = false;
			...
			hasMoreProduct = false;
			log.error("Exception ocurred while get list of voucher products  for accountNumber : "
					+ exchange.getProperty("accountNumber", String.class) + " and nickNameID : "
