...
public class ProcessUnlinkedInventory implements Processor {
	...
	public void process(Exchange exchange) throws Exception {
		...
		for (int i = 0; i < SKUs.length(); i++) {
			...
			inventoryList.add(processChild(exchange, attributes, SKUs.getJSONObject(i),
					SKU + "-" + variantCounterString, categoryID));
		}
	...
	private static BasicDBObject processChild(Exchange exchange, JSONObject attributes, JSONObject itemDetails,
			String SKU, String categoryID) throws Exception {
		...
		inventory.put("customSKU", itemDetails.getString("SellerSku"));
		...
		if (!exchange.getProperty("isInventoryEmpty", Boolean.class)) {
			...
			inventory.put("refrenceID", itemDetails.getString("SellerSku"));
		}
		...
		channel.put("refrenceID", itemDetails.getString("SellerSku"));
		...
		if (exchange.getProperty("itemHasVariants", Boolean.class)) {
			ArrayList<BasicDBObject> variantDetails = buildVariantsDetails(itemDetails,
	...
	private static ArrayList<BasicDBObject> buildVariantsDetails(JSONObject SKU, List<String> validOrderVariantList)
			throws JSONException {
